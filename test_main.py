...
from main import app, get_db, calculate_monthly_payment, calculate_loan_schedule, calculate_loan_summary
...
def test_create_loan(db_session):  
    ...
    response = client.post(
        ...
        json = {
            ...
            "user_ids": [
                test_data["user"][0].id, test_data["user"][1].id
            ]
        },
    )
...
def test_calculate_loan_summary():
    ...
    loan_schedule = calculate_loan_schedule(amount, annual_interest_rate, loan_term)
    loan_summary = calculate_loan_summary(schedule=loan_schedule, month_number=month_number, amount=amount)
...
def test_get_user_loans(db_session):
    ...
    assert data[0] == {'annual_interest_rate': 3.0, 'amount': 10000.0, 'loan_term': 12, 'id': test_data['loan'][0].id}
    assert data[1] == {'annual_interest_rate': 6.0, 'amount': 20000.0, 'loan_term': 24, 'id': test_data['loan'][1].id}

    response = client.get(f"/user/{test_data['user'][1].id}/loans")
    data = response.json()
    assert data[0] == {'annual_interest_rate': 6.0, 'amount': 20000.0, 'loan_term': 24, 'id': test_data['loan'][1].id}
...
def test_share_loan(db_session):
    ...
    response = client.post(f"/loan/{test_data['loan'][1].id}/share", json=[test_data['user'][2].id])
    assert response.status_code == 200
    response = client.get(f"/user/{test_data['user'][2].id}/loans")
    data = response.json()
    assert data[0] == {'annual_interest_rate': 6.0, 'amount': 20000.0, 'loan_term': 24, 'id': test_data['loan'][1].id}
